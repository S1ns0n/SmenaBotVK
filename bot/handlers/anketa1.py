...
async def projects_process(message: Message):
    ...
    ctx_storage.set("projects", projects)

    anketa_data = {
        "interests": ctx_storage.get("interests"),
        "profession": ctx_storage.get("profession"),
        "parents": ctx_storage.get("parents"),
        "fruits": ctx_storage.get("fruits"),
        "effects": ctx_storage.get("effects"),
        "groups": ctx_storage.get("groups"),
        "activities": ctx_storage.get("activities"),
        "factors": ctx_storage.get("factors"),
        "choice": ctx_storage.get("choice"),
        "projects": projects
    }
    ...
    • Интересы: {user_anketa['interests']}
    • Профессия: {user_anketa['profession']}
    • Родители: {user_anketa['parents']}
    • Плоды: {user_anketa['fruits']}
    • Эффекты: {user_anketa['effects']}
    • Группы: {user_anketa['groups']}
    • Активности: {user_anketa['activities']}
    • Факторы: {user_anketa['factors']}
    • Выбор: {user_anketa['choice']}
    • Проекты: {user_anketa['projects']}"""
