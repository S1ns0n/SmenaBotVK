...
class AnketAnalyzer:
    ...
    async def analyze_peer_anketas(self, peer_id: int) -> Optional[str]:
        """
        Анализирует все анкеты пользователя.
        Вопросы = ключи → нейронка видит контекст автоматически!
        """
        try:
            # Получаем все 3 анкеты
    ...
    def _build_prompt(self, anketas_data: Dict) -> str:
        """СУПЕР-ПРОСТОЙ промт: вопросы = ключи"""
...
🤖 ЗАДАЧА: Проанализируй ответы и дай:

1. **КОЛИЧЕСТВО БАЛЛОВ** по каждому разделу (считай по критериям из вопросов)
2. **ИНТЕРПРЕТАЦИЮ** по этим диапазонам (СЛОВО В СЛОВО):
...
3. **РЕКОМЕНДАЦИИ** по тренингам и развитию

Формат ответа:
**Раздел 2: X/32 баллов**
[точный текст по баллам]

**Раздел 3: Y/23 баллов**  
[точный текст по баллам]
...
**ИТОГОВАЯ РЕКОМЕНДАЦИЯ:**"""
    ...
    async def _get_ai_analysis(self, prompt: str) -> str:
        """Запрос к GPT с точными инструкциями"""
        response = self.client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {
                    "role": "system",
                    "content": """Ты профориентолог. 
Точно считай баллы по критериям из вопросов. 
Выдавай СЛОВО В СЛОВО интерпретации по диапазонам баллов.
Рекомендуй тренинги "Путь к самореализации" и "Формула мечты"."""
                },
